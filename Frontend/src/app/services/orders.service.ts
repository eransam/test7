...
export class OrdersService {
  ...
  async getAllOrders(): Promise<OrderModel[]> {
    if (store.getState().ordersState.orders.length === 0) {
      const orders = await firstValueFrom(
        this.http.get<OrderModel[]>(environment.ordersUrl)
      );
  ...
  async addOrder(order: OrderModel): Promise<OrderModel> {
    const addedOrder = await firstValueFrom(
      this.http.post<OrderModel>(environment.ordersUrl, order)
    );
  ...
  async countOrders(): Promise<number> {
    const count = await firstValueFrom(
      this.http.get<number>(environment.ordersCountUrl)
    );
  ...
  getMostRecentOrder(): OrderModel {
    ...
    if (loggedInUser !== null) {
      store.getState().ordersState.orders.forEach((o) => {
        if (o.user && o.user.userId !== loggedInUser.userId) {
          // if the order is not of the current usr
          return;
        }

        if (o.userId && o.userId !== loggedInUser.userId) {
